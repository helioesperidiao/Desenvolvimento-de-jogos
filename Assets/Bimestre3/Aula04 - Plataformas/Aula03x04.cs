...
public class Aula03x04 : MonoBehaviour {
	...
	void OnCollisionStay2D(Collision2D objetoPermaceTocando)    {
		...
		if (tag.Contains("mover") == true){
			print ("OnCollisionStay2D():" + tag);
			bool apertou = Input.GetKey(KeyCode.LeftControl);
			// se o usuário permanecer tocando no objeto com tag mover
			if (apertou == true){
				//o objeto com a tag mover terá como pai o transform do personagem
				objetoPermaceTocando.transform.parent = TransformPersonagem;
				//guarda o transform do elemento que o personagem está carregando.
				//o tranform desse objeto tem como pai o transform do personagem
	...
	void OnCollisionEnter2D(Collision2D objetoTocado ){
		...
		if (TagObjetoTocado.Contains("destrutivel2"))  {
			...
			GameObject quadradoBranco = objetoTocado.transform.GetChild(0).gameObject; // Pega o filho um

			GameObject circulo = objetoTocado.transform.GetChild(1).gameObject; // Pega o filho um

		
			circulo.transform.parent = null; // Desvincula os filhos
			quadradoBranco.transform.parent = null;  // Desvincula os filhos

			circulo.AddComponent<BoxCollider2D>(); // Adiciona um boxCollider2d ao circulo
			circulo.AddComponent<Rigidbody2D>();
			circulo.GetComponent<Rigidbody2D> ().velocity = new Vector2 (0, 10);
			circulo.tag = "Bonus1";
			//circulo.GetComponent<BoxCollider2D>().isTrigger = true; 

	
			Destroy(quadradoBranco.gameObject);
		...
		if (TagObjetoTocado.Contains("destrutivel1"))  {
				...
				GameObject circulo = objetoTocado.transform.GetChild(0).gameObject; // Pega o filho um

				//blocoBranco.transform.parent = null; // Desvincula os filhos
				circulo.transform.parent = null; // Desvincula os filhos
